...
// Features: Static hosting, Image→Video proxy (placeholder), Stripe/PayPal/Coinbase checkout, webhook stubs
...
const {
  PORT = 8080,
  NODE_ENV = 'production',
  STABILITY_API_KEY,                // (optional if you’re using a different provider)
  STRIPE_SECRET_KEY,
  STRIPE_PRICE_STARTER,             // e.g. price_123 for $9.99
  STRIPE_PRICE_CREATOR_MONTHLY,     // e.g. price_456 for $19.99 monthly
  STRIPE_PRICE_PRO_STUDIO,          // e.g. price_789 for $39.99 (or recurring)
  STRIPE_WEBHOOK_SECRET,            // for webhooks
  PUBLIC_BASE_URL = 'https://pulsemotionhub-video-app.onrender.com',
  PAYPAL_CLIENT_ID,                 // PayPal REST
  PAYPAL_CLIENT_SECRET,
  PAYPAL_ENV = 'live',              // 'sandbox' or 'live'
...
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    // TODO: Integrate your fal.ai / Kling / Stability call here.
...
app.post('/api/checkout/stripe/session', async (req, res) => {
  try {
    ...
    // If your plan is recurring, Stripe will detect from Price config (recurring vs one-time)
    const session = await stripe.checkout.sessions.create({
      mode: 'subscription', // for recurring prices; for one-time credits change to 'payment'
      line_items: [{ price, quantity: 1 }],
      allow_promotion_codes: true,
      automatic_tax: { enabled: true },
      // Enable wallets (Apple Pay/Google Pay/Link/Cash App Pay) automatically
...
app.post('/webhooks/stripe', bodyParser.raw({ type: 'application/json' }), (req, res) => {
  ...
  switch (event.type) {
    case 'checkout.session.completed':
      ...
      // TODO: provision credits / mark subscription active
      break;
...
app.post('/api/checkout/paypal/create', async (req, res) => {
  try {
    ...
    return res.json({ orderId: data.id, approvalUrl: approval?.href });
  } catch (err) {
...
// (Optional) PayPal capture route, if you want to capture server-side after return
app.post('/api/checkout/paypal/capture/:orderId', async (req, res) => {
  try {
    ...
    // TODO: grant credits/subscription
...
app.post('/api/checkout/coinbase/charge', async (req, res) => {
  try {
    if (!COINBASE_COMMERCE_API_KEY) return res.status(500).json({ error: 'Coinbase Commerce not configured.' });
    const plan = (req.query.plan || 'starter').toLowerCase();
    const { value, currency, name } = planAmount(plan);
    ...
    if (!charge.ok) {
      ...
      return res.status(500).json({ error: 'Failed to create crypto charge.' });
    }
    return res.json({ hosted_url: data?.data?.hosted_url, charge_code: data?.data?.code });
  } catch (err) {
...
app.post('/webhooks/coinbase', bodyParser.raw({ type: 'application/json' }), (req, res) => {
  try {
    ...
    // Handle: charge:confirmed -> grant credits
    res.json({ received: true });
  } catch (e) {
